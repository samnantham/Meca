...
app.controller('SDGsController', ['$scope', '$http', '$state', 'authServices', '$stateParams', 'webServices', 'utility', '$rootScope', '$timeout', '$filter', '$ngConfirm', 'toaster', function($scope, $http, $state, authServices, $stateParams, webServices, utility, $rootScope, $timeout, $filter, $ngConfirm, toaster) {
    ...
    $scope.addSDGsfiles = function(files) {
        $scope.errors = [];
        if ($scope.formData.sdgs_files.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $scope.formData.sdgs_files.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        ...
                        if ($rootScope.validextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.isfile = 1;
                                $scope.formData.sdgs_files.push(newobj);
                            } else {
                    ...
                    $scope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $scope.formData.sdgs_files.length) + ' files');
                }
            }
        } else {
    ...
    $scope.addSDGsDocuments = function(files) {
        $scope.errors = [];
        if ($scope.formData.sdgs_documents.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $scope.formData.sdgs_documents.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        var extn = files[i].name.split(".").pop();
                        if ($rootScope.validfileextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.filetype = files[i].type.split("/")[0];
                                newobj.isfile = 1;
                                $scope.formData.sdgs_documents.push(newobj);
                            } else {
                                $scope.errors.push(files[i].name + ' size exceeds 2MB.')
                            }
                        } else {
                    ...
                    $scope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $scope.formData.sdgs_documents.length) + ' files');
                }
            }
        } else {
            $scope.errors.push('You can add only maximum of ' + $rootScope.maxUploadFiles + ' files only');
    ...
    $scope.openaddModal = function() {
        ...
        $scope.formData.sdgs_files = [];
        $scope.formData.video_links = [];
        $scope.formData.sdgs_documents = [];
    ...
    $scope.pageChanged = function(newPage) {
        ...
        if (!$scope.pagedata[$scope.pageno]) {
            ...
            $scope.sdgslist = $scope.pagedata[$scope.pageno];
        }
    };
    ...
    $scope.getDatas = function() {
        $rootScope.loading = true;
        $scope.pageno = 1;
        $scope.url = 'sdgs/paginate/' + $scope.totalPerPage;
        $scope.getResults();
    };
