app.controller('MaasModalController', ['$timeout', '$state', '$stateParams', 'webServices', 'utility', '$rootScope', '$filter', function ($timeout, $state, $stateParams, webServices, utility, $rootScope, $filter) {
    $rootScope.videoData = {};
    $rootScope.documentData = {};
    if (!$rootScope.isEdititem) {
        $rootScope.formData = {};
        $rootScope.formData.type = '1';
        $rootScope.formData.maas_files = [];
        $rootScope.formData.video_links = [];
        $rootScope.formData.maas_documents = [];
        $rootScope.formData.document_links = [];
    } else {
        $rootScope.formData.type = $rootScope.formData.type.toString();
        $rootScope.formData.deleted_maas_files = [];
        $rootScope.formData.deleted_maas_documents = [];
    ...
    $rootScope.seterrorMsg = function () {
        ...
        $rootScope.errorData.description_errorMsg = 'Please add maas Study';
    ...
    $rootScope.addData = function (form) {
        ...
        if (form.$valid) {
            ...
            if($rootScope.isEdititem){
                webServices.putupload('maas/'+$rootScope.formData.id, $rootScope.formData).then(function (getData) {
                ...
                webServices.upload('maas', $rootScope.formData).then(function (getData) {
                    $rootScope.loading = false;
                    if (getData.status == 200) {
                        $rootScope.closeModalPopup();
                        $rootScope.$emit("showSuccessMsg", getData.data.message);
    ...
    $rootScope.addmaasfiles = function (files) {
        $rootScope.errors = [];
        if ($rootScope.formData.maas_files.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $rootScope.formData.maas_files.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        ...
                        if ($rootScope.validextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.isfile = 1;
                                $rootScope.formData.maas_files.push(newobj);
                            } else {
                    ...
                    $rootScope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $rootScope.formData.maas_files.length) + ' files');
                }
            }
        } else {
    ...
    $rootScope.addmaasDocuments = function (files) {
        $rootScope.errors = [];
        if ($rootScope.formData.maas_documents.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $rootScope.formData.maas_documents.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        var extn = files[i].name.split(".").pop();
                        if ($rootScope.validfileextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.filetype = files[i].type.split("/")[0];
                                newobj.isfile = 1;
                                $rootScope.formData.maas_documents.push(newobj);
                            } else {
                                $rootScope.errors.push(files[i].name + ' size exceeds 2MB.')
                            }
                        } else {
                    ...
                    $rootScope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $rootScope.formData.maas_documents.length) + ' files');
                }
            }
        } else {
            $rootScope.errors.push('You can add only maximum of ' + $rootScope.maxUploadFiles + ' files only');
    ...
    $rootScope.removeFile = function (key, data) {
        if (!data.isfile) {
            $rootScope.formData.deleted_maas_files.push(data);
        }
        $rootScope.formData.maas_files.splice(key, 1);
    }

    $rootScope.removeDocuments = function (key, data) {
        if (!data.isfile) {
            $rootScope.formData.deleted_maas_documents.push(data);
        }
        $rootScope.formData.maas_documents.splice(key, 1);
    }
