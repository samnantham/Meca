app.controller('KaizenModalController', ['$timeout', '$state', '$stateParams', 'webServices', 'utility', '$rootScope', '$filter', function ($timeout, $state, $stateParams, webServices, utility, $rootScope, $filter) {
    $rootScope.videoData = {};
    $rootScope.documentData = {};
    if (!$rootScope.isEdititem) {
        $rootScope.formData = {};
        $rootScope.formData.kaizen_files = [];
        $rootScope.formData.video_links = [];
        $rootScope.formData.kaizen_documents = [];
        $rootScope.formData.document_links = [];
        $rootScope.formData.type = '1';
    } else {
        $rootScope.formData.type = $rootScope.formData.type.toString();
        $rootScope.formData.deleted_kaizen_files = [];
        $rootScope.formData.deleted_kaizen_documents = [];
    ...
    $rootScope.seterrorMsg = function () {
        ...
        $rootScope.errorData.description_errorMsg = 'Please add Kaizen Study';
    ...
    $rootScope.addData = function (form) {
        ...
        if (form.$valid) {
            ...
            if($rootScope.isEdititem){
                webServices.putupload('kaizen/'+$rootScope.formData.id, $rootScope.formData).then(function (getData) {
                ...
                webServices.upload('kaizen', $rootScope.formData).then(function (getData) {
                    $rootScope.loading = false;
                    if (getData.status == 200) {
                        $rootScope.closeModalPopup();
                        $rootScope.$emit("showSuccessMsg", getData.data.message);
    ...
    $rootScope.addkaizenfiles = function (files) {
        $rootScope.errors = [];
        if ($rootScope.formData.kaizen_files.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $rootScope.formData.kaizen_files.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        ...
                        if ($rootScope.validextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.isfile = 1;
                                $rootScope.formData.kaizen_files.push(newobj);
                            } else {
                    ...
                    $rootScope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $rootScope.formData.kaizen_files.length) + ' files');
                }
            }
        } else {
    ...
    $rootScope.addkaizenDocuments = function (files) {
        $rootScope.errors = [];
        if ($rootScope.formData.kaizen_documents.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $rootScope.formData.kaizen_documents.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        var extn = files[i].name.split(".").pop();
                        if ($rootScope.validfileextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.filetype = files[i].type.split("/")[0];
                                newobj.isfile = 1;
                                $rootScope.formData.kaizen_documents.push(newobj);
                            } else {
                                $rootScope.errors.push(files[i].name + ' size exceeds 2MB.')
                            }
                        } else {
                    ...
                    $rootScope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $rootScope.formData.kaizen_documents.length) + ' files');
                }
            }
        } else {
            $rootScope.errors.push('You can add only maximum of ' + $rootScope.maxUploadFiles + ' files only');
    ...
    $rootScope.removeFile = function (key, data) {
        if (!data.isfile) {
            $rootScope.formData.deleted_kaizen_files.push(data);
        }
        $rootScope.formData.kaizen_files.splice(key, 1);
    }

    $rootScope.removeDocuments = function (key, data) {
        if (!data.isfile) {
            $rootScope.formData.deleted_kaizen_documents.push(data);
        }
        $rootScope.formData.kaizen_documents.splice(key, 1);
    }
