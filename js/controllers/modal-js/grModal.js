app.controller('GRModalController', ['$timeout', '$state', '$stateParams', 'webServices', 'utility', '$rootScope', '$filter', function ($timeout, $state, $stateParams, webServices, utility, $rootScope, $filter) {
    ...
    $rootScope.addData = function (form) {
        ...
        if (form.$valid) {
            ...
            if($rootScope.isEdititem){
                ...
                webServices.upload('gr', $rootScope.formData).then(function (getData) {
                    $rootScope.loading = false;
                    if (getData.status == 200) {
                        $rootScope.closeModalPopup();
                        $rootScope.$emit("showSuccessMsg", getData.data.message);
    ...
    $rootScope.addgrfiles = function (files) {
        $rootScope.errors = [];
        if ($rootScope.formData.gr_files.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $rootScope.formData.gr_files.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        ...
                        if ($rootScope.validextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.isfile = 1;
                                $rootScope.formData.gr_files.push(newobj);
                            } else {
                    ...
                    $rootScope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $rootScope.formData.gr_files.length) + ' files');
                }
            }
        } else {
    ...
    $rootScope.addgrDocuments = function (files) {
        $rootScope.errors = [];
        if ($rootScope.formData.gr_documents.length < $rootScope.maxUploadFiles) {
            if (files && files.length) {
                if (($rootScope.maxUploadFiles - $rootScope.formData.gr_documents.length) >= files.length) {
                    for (var i = 0; i < files.length; i++) {
                        var extn = files[i].name.split(".").pop();
                        if ($rootScope.validfileextensions.includes(extn.toLowerCase())) {
                            if (files[i].size <= $rootScope.maxUploadsize) {
                                ...
                                newobj.filetype = files[i].type.split("/")[0];
                                newobj.isfile = 1;
                                $rootScope.formData.gr_documents.push(newobj);
                            } else {
                                $rootScope.errors.push(files[i].name + ' size exceeds 2MB.')
                            }
                        } else {
                    ...
                    $rootScope.errors.push('You can now upload only ' + ($rootScope.maxUploadFiles - $rootScope.formData.gr_documents.length) + ' files');
                }
            }
        } else {
            $rootScope.errors.push('You can add only maximum of ' + $rootScope.maxUploadFiles + ' files only');
    ...
    $rootScope.removeFile = function (key, data) {
        if (!data.isfile) {
            $rootScope.formData.deleted_gr_files.push(data);
        }
        $rootScope.formData.gr_files.splice(key, 1);
    }

    $rootScope.removeDocuments = function (key, data) {
        if (!data.isfile) {
            $rootScope.formData.deleted_gr_documents.push(data);
        }
        $rootScope.formData.gr_documents.splice(key, 1);
    }
